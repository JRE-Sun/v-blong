    ...
    <div :style="isOpenTree ? '':'display:none'" class="title-tree" @click.prevent.stop="clickTreeBg">
        ...
        <div class="main" :class="isOpenTreeMain ? 'open' : 'close' ">
            ...
            <div class="title-tree__bd">
                <ol>
                    <li @click.prevent.stop="clickTreeLi(index)" v-for="(tree,index) in treeList">{{ tree }}</li>
                </ol>
            </div>
        </div>
    </div>
    ...
    export default {
        ...
        methods: {
            clickTreeLi(index) {
                ...
                let top = document.querySelector(`#${this.idPrefix + index}`).offsetTop - scrollTop;
            ...
            openTitleTree(data) {
                ...
                Array.from(document.querySelectorAll(`${target} h1`)).forEach((item, index) => {
