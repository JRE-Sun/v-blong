        ...
        <div id="primary">
            <article class="post">
                ...
                <div id="editor" class="post-content">
                    <mavon-editor :toolbarsFlag="false" :subfield="false" :editable="false" :defaultOpen="defaultOpen" v-model="article['art_content']" />
                </div>
                <footer class="post-footer">
                    <div class="post-tags">
                        <router-link :to="{name:'list',query:{cate_id:article['cate_id'],key:key}}">{{ article['cate_name'] }}
                        </router-link>
                    </div>
                    <nav v-show="isEmpty(nextInfo.length) || isEmpty(preInfo.length)" class="post-nav">
                        <router-link v-show="isEmpty(preInfo.length)" class="prev" :to="{name:'article',query:{art_id:preInfo['art_id'],key:key}}">
                            <i class="iconfont icon-left"></i>
                            <span class="prev-text nav-default">{{ preInfo['art_title'] }}</span>
                        </router-link>
                        <router-link v-show="isEmpty(nextInfo.length)" class="next" :to="{name:'article',query:{art_id:nextInfo['art_id'],key:key}}">
...
export default {
    ...
    computed: {
        ...mapState(["api"]),
        getDate() {
            return this.base.formatDate(
                this.article["art_addtime"],
                "y-m-d h:n:s"
            );
        },
        key() {
    ...
    methods: {
        ...
        getAjaxInfo(artID) {
            ...
            setTimeout(() => {
                ...
                if (artList == null) {
                    this.API.get(
                        "index.php/index/article/detail/art_id/" + artID,
                        res => {
